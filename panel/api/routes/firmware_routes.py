...
def post_pick(body):
    ...
    if len(extensions) > 1:
        return respond(400, {
            "error": i18n.t("error.mixedFileTypes")})
    ...
    selection = firmware.select_file(
        [device.id for device in targets], chosen,
        str(body.get("version", "")), set_no=inventory.set_no)
    return respond(200, {"files": selection, "deviceCount": len(targets),
                         "selectedCount": len(firmware.selections(
                             set_no=inventory.set_no))})


def post_version(body):
    # Only the target version changes; the selected file is untouched.
    inventory = inventory_for(body.get("set"))
    devices = target_devices(inventory, body, "fw")
    selection = firmware.set_target_version(
        [device.id for device in devices], str(body.get("version", "")),
        set_no=inventory.set_no)
    return respond(200, {"files": selection})
...
def post_file(body):
    ...
    targets = _installable(inventory, body)
    selection = firmware.select_file(
        [device.id for device in targets], path,
        str(body.get("version", "")), set_no=inventory.set_no)
    return respond(200, {"files": selection, "deviceCount": len(targets),
                         "selectedCount": len(firmware.selections(
                             set_no=inventory.set_no))})
...
POST = {
    ...
    "/api/firmware/version": post_version,
